...
export class WebSocketServer {
  ...
  /** Boş = kimlik doğrulama kapalı (geriye uyum). */
  ...
  setAuthToken(token: string | undefined | null): void {
    this.authToken = (token || "").trim();
    this.log(
      this.authToken
        ? "WebSocket auth token enabled"
        : "WebSocket auth token disabled"
    );
  }
  ...
  private validateAuthToken(candidate: unknown): boolean {
    if (!this.authToken) {
      return true;
    }
    return typeof candidate === "string" && candidate === this.authToken;
  }

  private isWsAuthenticated(ws: WebSocketClient): boolean {
    return (ws as any).authenticated === true;
  }
  ...
  async start(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      this.wss!.on("connection", (ws: WebSocketClient) => {
        ...
        (ws as any).authenticated = !this.authToken;
        ...
        ws.on("message", (message: Buffer) => {
          ...
          try {
            ...
            const wsClientId = (ws as any).clientId as string;
            const stableClientId =
              typeof parsed.clientId === "string" &&
              parsed.clientId.trim().length > 0
                ? parsed.clientId.trim()
                : wsClientId;
            (ws as any).clientId = stableClientId;
            parsed.clientId = stableClientId;

            if (parsed.type === "auth") {
              const ok = this.validateAuthToken(
                parsed.token ?? parsed.authToken
              );
              (ws as any).authenticated = ok;
              this.sendToClient(
                ws,
                JSON.stringify({
                  type: "auth_result",
              ...
              if (!ok) {
                this.log(`Auth failed for ${clientId}`, "warn");
              }
              return;
            }

            const tokenFromMessage = parsed.authToken ?? parsed.token;
            if (
              this.authToken &&
              !this.isWsAuthenticated(ws) &&
              !this.validateAuthToken(tokenFromMessage)
            ) {
              this.sendToClient(
                ws,
                JSON.stringify({
                  ...
                  clientId,
                  error:
                    "Auth token gerekli. Bağlantıdan sonra { type: auth, token } gönderin.",
                })
              );
              return;
            }

            if (this.authToken && this.validateAuthToken(tokenFromMessage)) {
            ...
            if (this.authToken && !this.isWsAuthenticated(ws)) {
              this.sendToClient(
                ws,
                JSON.stringify({
                  type: "auth_required",
                  success: false,
                  clientId,
                  error: "Geçersiz JSON veya auth gerekli",
                })
              );
              return;
            }
